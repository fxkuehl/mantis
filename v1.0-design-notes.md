...
![Render](./assets/v1.0-renders/1152/v1.0-mantis.jpg)
...
Choc v2 switches are mostly pin-compatible with Choc v1. More v2 switches
are becoming available. Supporting both would be preferred. This requires
accommodations on the PCB (larger central stem and different plastic pin),
plate (0.15mm larger cutout, maybe within tolerance) and keycaps (different
stem, slightly taller switch). See
[differences](https://showcase.beekeeb.com/kailh-choc-v2-key-switches/).
...
plate like on v0.2, but covered up by top deck. Vertical space available
...
The larger trackball leaves not enough room for dedicated mouse buttons.
...
To track the ball at the right angle, the sensor would need to be as far
...
For lower-profile version use 35mm trackpad instead of trackball.

[Datasheet](https://www.mouser.ca/pdfdocs/TM035035_SPI-I2C-PINNTrackpad_SPEC1-21.pdf)
...
This is considered for Trackpad builds. The trackball doesn't leave enough
touch surface for the mouse buttons.
...
FFC connector: [Molex Datasheet](https://www.molex.com/en-us/products/part-detail-pdf/5051101297?display=pdf) [Molex Drawings](https://www.molex.com/content/dam/molex/molex-dot-com/products/automated/en-us/salesdrawingpdf/505/505110/5051101297_sd.pdf) [Mouser HRS Datasheet](https://www.mouser.com/datasheet/3/731/1/FH33-12S-0.5SH%2810%29_CL0580-1302-4-10_2DDrawing_0000990713.pdf)

## Battery and display connectors

The battery and display connectors can use JST-PH connectors, which are easy
to source. For a low-profile, non-gasket mounted future version, the display
can also be mounted with a normal 5-pin 2.54mm pitch header.

## ESD protection

Nice!nano controllers are notorious for ESD damage when connected to USB,
because GND is not connected to the USB housing. This can be partly mitigated
with a ferrite bead between the ground plane and the controller's ground pins.
...
* Column staggered key wells
* 30° hand rotation
* 15° tenting
* Support for wired builds with RP2040 and per-key LEDs

New in v1.0:

* Integrated trackball, 34mm, static bearings, PMW3610 sensor
* 3D-printed high-profile enclosed case hiding switches
* 2 additional pinky keys, 2 fewer thumb keys (about 22mm wider)
* Saddle-shaped key profile
* Compatible with Choc v1 and v2 switches
* Hot-swap
* Gasket-mounted switch plates
* Foam or cork layers for sound dampening
* Internal use of VIK connectors/cables/signaling (should allow for future
  version with Cirque trackpad using the same PCBs)
* Intended for PCB-assembly, no manual SMD soldering required
* Support for wireless builds with nice!nano (or clones) and nice!view display
  (does not support LEDs at the same time as display)
* ZMK firmware
* Low-profile design: 2mm lower than v0.3, including the trackball (6mm lower without trackball)
    - counter-sunk screws
    - recessed feet
    - internal recesses in base plate for PCB components with 1mm gasket compression
    - thinner PCBs (1.2mm) and switch plates
    - angled sensor mount for low trackball position
